			...
			<div class="controls">
				<div class="control-group">
					...
					<fieldset>
						...
						<div id="allpermissions" class="control-group" style="float:left; width:180px; border:1px solid #dddddd; padding:12px; margin-right:12px; border-radius:4px;">
							<label class="control-group">{{ $permission['label'] }}</label>

							
							<div class="btn-group" data-toggle="buttons">
	                            <label class="btn btn-mini btn-default">
	                                <input type="radio" name="permissions[{{ $permission['permission'] }}]" value="1" {{ (array_get($userPermissions, $permission['permission']) == 1 ? ' checked="checked"' : '') }}>
	                            Allow 
	                            </label>
	                            <label class="btn btn-mini btn-default">
	                                <input type="radio" name="permissions[{{ $permission['permission'] }}]" value="0" {{ ( ! array_get($userPermissions, $permission['permission']) ? ' checked="checked"' : '') }}>
	                            Deny 
	                            </label>

	                            @if ($permission['can_inherit'])
								<label class="btn btn-mini btn-default">
									<input type="radio" value="0" name="permissions[{{ $permission['permission'] }}]"{{ ( ! array_get($userPermissions, $permission['permission']) ? ' checked="checked"' : '') }}>
									Inherit
								</label>
								@endif
