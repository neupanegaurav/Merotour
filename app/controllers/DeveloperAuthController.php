...
class DeveloperAuthController extends BaseController {
	...
	public function index()
	{
		...
		if (Sentry::check()) {
			...
			return View::make('developer-portal.index');

		} else {

		// Show the page
		return View::make('developer-portal.signin');
       ...
       try
		{
			...
			$developer = $group->findById(9);

			if ($user->inGroup($admin) or $user->inGroup($developer)) { 

				return Redirect::back()->with('success', 'Login successful, welcome to the Blackeye Developer Portal!');

			} else {

            	return Redirect::back()->with('error', 'Insufficient privilidges. You must be a developer to access this portal.');
        	}
