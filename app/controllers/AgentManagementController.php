...
class AgentManagementController extends AdminController {
    ...
    $userprovider = Sentry::getUserProvider();
  	...
  	$users = $user_list_raw->filter(function($user) use($userprovider, $agent)
    { 
		...
		// Show the page
		return View::make('backend.agent_management.index', compact('users'));
	}
	...
	public function postIndex()
	{
  	...
  	$users = $users->filter(function($user) use($userprovider, $agent)
    { 
        $current_user = $userprovider->findById($user->id);
	...
	public function getCreate()
	{
		...
		// Show the page
		return View::make('backend.agent_management.create', compact('groups', 'selectedGroups', 'permissions', 'selectedPermissions'));
	}
	...
	public function postCreate()
	{
		...
		try
		{
			...
			{
				// Assign the agent group to this user
				...
				$success = Lang::get('admin/agents/message.success.create');
			...
			$error = Lang::get('admin/agents/message.error.create');
		...
		{
			$error = Lang::get('admin/agents/message.user_login_required');
		}
		catch (PasswordRequiredException $e)
		{
			$error = Lang::get('admin/agents/message.user_password_required');
		}
		catch (UserExistsException $e)
		{
			$error = Lang::get('admin/agents/message.user_exists');
		}
	...
	public function getEdit($id = null)
	{
		...
		{
			// Prepare the error message
			$error = Lang::get('admin/agents/message.user_not_found', compact('id'));
		...
		// Show the page
		return View::make('backend.agent_management.edit', compact('user', 'groups', 'userGroups', 'permissions', 'userPermissions'));
	}
	...
	public function postEdit($id = null)
	{
		...
		try
		{
			...
			{
				// Prepare the success message
				$success = Lang::get('admin/agents/message.success.update');
			...
			// Prepare the error message
			$error = Lang::get('admin/agents/message.error.update');
		}
		catch (LoginRequiredException $e)
		{
			$error = Lang::get('admin/agents/message.user_login_required');
	...
	public function getDelete($id = null)
	{
		try
		{
			...
			{
				// Prepare the error message
				$error = Lang::get('admin/agents/message.error.delete');
			...
			// Prepare the success message
			$success = Lang::get('admin/agents/message.success.delete');
	...
	public function getClass()
	{
		...
		// Show the page
		return View::make('backend.agent_management.class', compact('entries'));
	}
	...
	public function getRestore($id = null)
	{
		try
		{
			...
			// Prepare the success message
			$success = Lang::get('admin/agents/message.success.restored');

			// Redirect to the user management page
			return Redirect::route('agent-management')->with('success', $success);
		}
		catch (UserNotFoundException $e)
		{
			// Prepare the error message
			...
			// Redirect to the user management page
			return Redirect::route('agent-management')->with('error', $error);
		}
	}
